...
private:
	...
	bool ProcessInstruction(std::map<char, int_fast64_t>& registers, std::string instruction, std::queue<int_fast64_t>& sendFrequency, int& currentIndex, int& sendCont, bool isPart2);
	...
	void Jgz(std::map<char, int_fast64_t> registers, char regCondition, int offsetJump, int& currentIndex);
